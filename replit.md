...
- `artifacts/intune-script-builder` — React + Vite web app: "Intune PowerShell Script Builder & Visualizer". Dark Microsoft/Azure-inspired theme. All client-side. Helps IT admins generate Intune Proactive Remediation scripts (detection, remediation, rollback) with execution-flow visualization and a deployment readiness checklist.
  - **Hosting**: published to GitHub Pages from `MikkelsenBrenno/MikkelsenBrenno.Powershell.Github.io` via `.github/workflows/deploy-pages.yml` (runs on push to `main` and `workflow_dispatch`). The workflow installs deps with pnpm 10 / Node 24, runs `pnpm --filter @workspace/intune-script-builder run build` with `BASE_PATH=/MikkelsenBrenno.Powershell.Github.io/`, copies `dist/public/index.html` to `404.html` for the SPA deep-link fallback, and publishes via `actions/upload-pages-artifact` + `actions/deploy-pages`. Live URL: `https://mikkelsenbrenno.github.io/MikkelsenBrenno.Powershell.Github.io/`. To enable the first deploy, set the repo's **Settings → Pages → Build and deployment → Source** to **GitHub Actions**.
  - `vite.config.ts` uses `defineConfig(async ({ command }) => …)` so `PORT` is only required for `command === "serve"` (dev/preview), and `BASE_PATH` is optional for builds (defaults to `/MikkelsenBrenno.Powershell.Github.io/`). Replit dev still requires both env vars and is unchanged.
